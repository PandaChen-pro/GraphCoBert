    ...
    for idx in range(args.epochs): 
        ...
        for step, batch in enumerate(bar):
            ...
            if not (torch.isnan(loss) or torch.isinf(loss)):
                ...
                logger.warning("Logits: %s", logits)
                logger.warning("Labels: %s", labels)
                logger.warning("Loss tensor: %s", loss)
            if avg_loss==0:
                avg_loss=tr_loss
...
def evaluate(args, model, tokenizer, eval_when_training=False):
    ...
    # Use subset during training to speed up evaluation
    if eval_when_training:
        # Use only 20% of eval data during training for faster evaluation
        subset_size = min(len(eval_dataset) // 5, 5000)  # Max 5000 samples
